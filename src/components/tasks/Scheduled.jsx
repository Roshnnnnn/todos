...
const Scheduled = () => {
  ...
  const showPagination = hasMoreTasks();
    ...
    <div className="bg-slate-500 min-h-screen text-black">
      ...
      <div className="container mx-auto py-8">
        <div className="flex justify-center items-center">
          <h1 className="text-4xl my-4">Scheduled's Tasks</h1>
        </div>

        {scheduledTasks.length === 0 ? (
          <div className="flex justify-center items-center text-xl font-semibold mb-8">
            <NoTask />
          </div>
        ) : (
          <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[4rem] m-4">
            {scheduledTasks.map((task, index) => (
              <div key={index}>
                <li
                  key={index}
                  className="bg-white p-6 rounded-md shadow-md flex flex-col justify-between mb-8 md:mb-0"
                >
                  <div className="mb-4">
